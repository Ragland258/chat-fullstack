...
bool RedisMgr::VerifyLoginToken(
    const std::string& email,
    const std::string& token)
{
    ...
    {
        std::cerr
            << "[VerifyLoginToken] email or token is empty"
            << std::endl;
    ...
    std::cout
        << "[VerifyLoginToken] redis key="
        << redisKey
        << std::endl;

    std::string storedToken;

    if (!Get(redisKey, storedToken))
    {
        std::cerr
            << "[VerifyLoginToken] token key not found or Redis error, key="
            << redisKey
            << std::endl;
    ...
    {
        std::cerr
            << "[VerifyLoginToken] token mismatch"
            << ", stored length=" << storedToken.size()
            << ", request length=" << token.size()
            << std::endl;
    ...
    std::cout
        << "[VerifyLoginToken] token verified"
        << std::endl;
